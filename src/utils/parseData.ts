...
import { convertRawViewstoString, parseVideoDuration, timeSince } from "utils";
...
export const parseData = async (items: any[]) => {
  try {
    ...
    const {
      ...
      `${YT_API_URL}/channels?part=snippet,contentDetails&id=${channelIds.join(
        ","
      )}&key=${API_KEY}`
    );
    ...
    const {
      ...
      `${YT_API_URL}/videos?part=contentDetails,statistics&id=${videoIds.join(
        ","
      )}&key=${API_KEY}`
    );
